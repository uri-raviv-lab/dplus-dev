...
Graph::Graph(RECT area, 
	...
	bLogScaleY = logScaleY;
	bLogScaleX = logScaleX;
	...
	x = new vector<double>[1];
	y = new vector<double>[1];
	cx = new vector<double>[1];
	cy = new vector<double>[1];
	x[0] = tx;
	y[0] = ty;
	...
	if(ty.size() == 0) {
		...
		for(unsigned int i = 0; i < tx.size(); i++) {
			if((ty[i] > ymax && !ty[i].IsInfinity(ty[i])) || ymax != ymax)
				ymax = ty[i];
			if((ymin > ty[i] && !ty[i].IsInfinity(ty[i])) || ymin != ymin)
				ymin = ty[i];
		}
...
void Graph::GeneratePixelGraphs(int id) {
...
bool Graph::ChangeColor(int i, COLORREF col) {
	if(i >= this->numGraphs)
		return false;
...
bool Graph::ChangeColor(int i, int r, int g, int b) {
	if(i >= this->numGraphs)
		return false;
...
void Graph::SetDefaultScale(const vector<double> &tx, const vector<double>& ty) {
	...
	if(tx.back() == tx[0])
		...
		if(bLogScaleX)
			defscaleX = graphWidth / (log10(tx.back()) - log10(tx[isNotReal(log10(tx[0])) ? 1 : 0]));
		else
			defscaleX = graphWidth / (tx.back() - tx[0]);
	}
	defoffX = (bLogScaleX ? (log10(tx[isNotReal(log10(tx[0])) ? 1 : 0])) : tx[0]);
...
void Graph::Modify(int num, vector<double> tx, 
				   vector<double> ty) {
	mutex.WaitOne();
	x[num] = tx;
	y[num] = ty;
	...
	// Avi: this makes the first graph be redrawn on it's own scale.
	//	if there are other graphs that have already been drawn, they
	//	remain on their own scale...
	//if(num == 0)
	//	SetDefaultScale(x[num], y[num]);
	// If the comment remains, modifying the first graph will not resize the drawn area
...
void Graph::Legend(const std::vector<std::string> &keys) {
	mutex.WaitOne();
	*legendKeys = keys;
	bLegend = true;
	mutex.ReleaseMutex();
...
void Graph::Highlight(int num, int startX, int endX, COLORREF color) {
	mutex.WaitOne();
	highlightInd = num;
...
void Graph::HighlightSingle(int num, int startX, int endX, COLORREF color) {
	mutex.WaitOne();
	highlightInd = num;
...
void Graph::Mask(int num, int startX, int endX, COLORREF color) {
	mutex.WaitOne();
...
int Graph::GetHighlightStart(std::vector<std::pair<int, int>> & ind) {
	if(ind.size() == 0)
		return -1;

	int res = ind.at(0).first;
	for(int i = 1; i < (int)ind.size(); i++)
		res = min(res, ind.at(i).first);
	return res;
}

int Graph::GetHighlightEnd(std::vector<std::pair<int, int>> & ind) {
	if(ind.size() == 0)
		return -1;

	int res = ind.at(0).second;
	for(int i = 1; i < (int)ind.size(); i++)
		res = max(res, ind.at(i).second);
	return res;
}

bool Graph::LogScaleY() {
	return bLogScaleY;
}

bool Graph::LogScaleX() {
	return bLogScaleX;
}

void Graph::SetScale(int axis, GraphScale gs) {
	if(axis == 0)	// y-axis
		SetScale(gs,  (bLogScaleX ? SCALE_LOG : SCALE_LIN)/*, 0*/);
	else
		SetScale((bLogScaleY ? SCALE_LOG : SCALE_LIN), gs/*, 0*/);
}
void Graph::SetScale(GraphScale gsY, GraphScale gsX) {
	bLogScaleX = (gsX == SCALE_LOG);

	bLogScaleY = (gsY == SCALE_LOG);

	RECT newArea;
	newArea.left = xoff;
	newArea.top = yoff;
	newArea.bottom = height + yoff;
	newArea.right = width + xoff;

	Resize(newArea);
}

void Graph::Resize(RECT area) {
	xoff = area.left;
	yoff = area.top;
	height = area.bottom - area.top;
	width = area.right - area.left;
...
void Graph::SZoom(int startX, int startY) {
	zoomX = startX;
	zoomY = startY;
	bRedrawZoom = true;
}

void Graph::EZoom(int endX, int endY) {
	endX  -= graphOffsetX;
	endY  -= graphOffsetY;
	zoomX -= graphOffsetX;
	zoomY -= graphOffsetY;
...
void Graph::FitToAllGraphs() {
	if((y->size() == 0) || (x->size() == 0) || numGraphs < 1)
		return;
	double ymin = y[0][0];
	double ymax = y[0][0];
	double xmin = x[0].back();
	double xmax = x[0][0];
...
std::pair<double, double> Graph::PointToData(int x, int y) {
	double tempx = x - graphOffsetX, tempy = y - graphOffsetY;
...
void Graph::PaintTicks(Graphics ^g) {
	int res = tickres;
	...
	Font ^textFont = gcnew Font(gcnew FontFamily("Arial"), 7, 
	...
	if(bLogScaleX && bXTicks && numGraphs > 0) {
		double xmax = x->back(), xmin = x->front(), xscale = defscaleX;
		int pos, j;
		double orders;

		//Check to make sure that the q values are shown in the graph
		for(j = 0; (unsigned)j < cx->size(); j++) {
			if(cx->at(j) >= 0.0 && (!bLogScaleX || x->at(j) > 0.0)) {
				xmin = x->at(j);
				break;
			}
		}
		for(int end = cx->size() - 1; end > j; end--) {
			if(cx->at(end) / 1.00001 <= graphWidth ) {
				xmax = x->at(end);
				break;
			}
		}		
...
void Graph::PaintBackground(Graphics ^g) {
	g->Clear(Color::White);
	g->DrawRectangle(gcnew Pen(Color::Black), 0, 0, width - 1, height - 1);
}

void Graph::PaintZoom(Graphics ^g, int mouseX, int mouseY) {
...
void Graph::PaintLabels(Graphics ^g) {
	// Title
	if(title->Length > 0) {
		StringFormat ^sf = gcnew StringFormat();
		sf->Alignment = StringAlignment::Center;
		g->DrawString(title, 
			gcnew Font(gcnew FontFamily("Arial"), 8, 
			FontStyle::Regular, GraphicsUnit::Point, 
					   DEFAULT_CHARSET),
			gcnew SolidBrush(Color::Black),
...
void Graph::PaintPeak(Graphics ^g, int mouseX, int mouseY) {
...
void Graph::PaintCrop(Graphics ^g, int mouseX) {
...
bool Graph::IsInHighlight(int graphNum, int index) {
	if(graphNum == 0) {
...
void Graph::DrawGraphFromVectors(int graphNum, Graphics ^g, Pen ^gcolor, Pen ^hcolor, const std::vector<double>& x, 
...
void Graph::PaintGraph(Graphics ^g, int w, int h) {
	if(w <= 0 || h <= 0)
		return;
...
void Graph::PaintGrid(Graphics ^g) {
	int res = tickres;
	...
	if(LogScaleX() && numGraphs > 0) {
		double xmin = x->front(), xmax = x->back(), orders;
		int pos, j;
...
void Graph::PaintLegend(Graphics ^g) {
	Font ^textFont = gcnew Font(gcnew FontFamily("Arial"), 8, 
							   FontStyle::Regular, GraphicsUnit::Point, 
							   DEFAULT_CHARSET);
...
void Graph::Repaint(Graphics ^g, int mouseX, int mouseY) {
	if(width <= 0 || height <= 0)
		return;
...
void Graph::ExportImage(IntPtr hParent) {	
	...
	str << clrToString(sfd->FileName);

	bool bAppendExt = !sfd->FileName->Contains(".");
	...
	bitmap->Save(gcnew String(str.str().c_str()), format);

	mutex.ReleaseMutex();
}

void Graph::DrawPeakOutline(int xi, int yi, int xf, int yf) {
...
void Graph::SetDrawPeak(bool bDraw) {
...
void Graph::SetCropping(bool _bCrop, bool _bMask) {
	bRedrawCrop = bCrop = _bCrop;
	bMask = _bMask;
}

bool Graph::GetGraphVisibility(int num) {
	return vis->at(num);
}
void Graph::SetGraphVisibility(int num, bool visible) {
	vis->at(num) = visible;
}

// Change to scientific form
std::pair<double, int> Graph::toSci(double x) {

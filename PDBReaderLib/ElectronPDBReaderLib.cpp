...
#pragma warning( pop )
	...
	template<class FLOAT_TYPE>
	bool ElectronPDBReader::electronPDBReaderOb<FLOAT_TYPE>::getHasAnomalous()
	{
...
#pragma warning( pop )
	...
	)
	{
		...
		for (unsigned int i = 0; i < vecSize; i++) {
			...
			ElectronPDBReader::float4 loc = { float(entries[i].x), float(entries[i].y), float(entries[i].z), 0.f };
			atmLocs[i] = loc;

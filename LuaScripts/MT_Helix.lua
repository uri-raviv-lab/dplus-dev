Information = {
	Name = "Super Helical Left Hand MT Helix with a twist - order of rotation changed, skip first n points",
	Type = "Symmetry",
	NLP = 1,
	MinLayers = 10,
	MaxLayers = 10,
...
function Populate(p, nlayers)	 
	if (p == nil or nlayers ~= 10 or table.getn(p[1]) ~= 1) then				
		error("Parameter matrix must be 10x1, it is " .. nlayers .. "x" .. table.getn(p[1]));
	end
	...
	superHelicalPitch	= p[8][1];
	RingTwistAlpha		= p[9][1];
	RingTwistBeta		= p[10][1];

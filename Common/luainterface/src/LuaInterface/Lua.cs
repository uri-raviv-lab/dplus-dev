...
{    
    ...
    public class Lua : IDisposable
	{
        ...
        void ThrowExceptionFromError(int oldTop, bool bCompileError)
        {
            ...
            if (bCompileError)
            {
                if (err == null) err = "Script Compilation Error";
                throw new LuaCompileException(err.ToString(), "");
            }
        ...
        public LuaFunction LoadString(string chunk, string name)
        {
            ...
            try
            {
                if (LuaDLL.luaL_loadbuffer(luaState, chunk, name) != 0)
                    ThrowExceptionFromError(oldTop, true);
            }
            ...
            if (LuaDLL.luaL_loadbuffer(luaState, chunk, "chunk") == 0)
            {
                ...
                try
                {
                    if (LuaDLL.lua_pcall(luaState, 0, -1, 0) == 0)
                        ...
                        ThrowExceptionFromError(oldTop, false);
                }
                finally { executing = false; }
            }
            else
        ...
        public int GetTableLength(LuaTable table)
        {
            translator.push(luaState, table);
            int length = LuaDLL.luaL_getn(luaState, -1);
      ...
      private void DebugHookCallback(IntPtr luaState, IntPtr luaDebug)
      {
         try
         {
             // Tal: PtrToStructure throws an AccessViolationException for some reason.
             //      This is a workaround.
             LuaDebug ld = new LuaDebug();// = (LuaDebug)System.Runtime.InteropServices.Marshal.PtrToStructure(luaDebug, typeof(LuaDebug));
             GetStack(0, out ld);
             //GetInfo("nSl", ref ld);
             GetInfo("l", ref ld); // <-- Optimization since we don't need the other fields for now
